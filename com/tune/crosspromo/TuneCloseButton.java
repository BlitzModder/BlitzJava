package com.tune.crosspromo;

import android.content.Context;
import android.content.res.Resources;
import android.graphics.Bitmap;
import android.graphics.BitmapFactory;
import android.graphics.Rect;
import android.graphics.drawable.BitmapDrawable;
import android.graphics.drawable.Drawable;
import android.util.Base64;
import android.util.DisplayMetrics;
import android.view.TouchDelegate;
import android.view.View;
import android.view.View.OnClickListener;
import android.widget.FrameLayout;
import android.widget.FrameLayout.LayoutParams;
import android.widget.ImageButton;

class TuneCloseButton
  extends FrameLayout
{
  private TuneAdActivity adActivity;
  private ImageButton close;
  View.OnClickListener closeListener = new View.OnClickListener()
  {
    public void onClick(View paramAnonymousView)
    {
      TuneAdClient.logClose(TuneCloseButton.this.adActivity.adView, TuneCloseButton.this.adActivity.adParams);
      TuneCloseButton.this.adActivity.finish();
    }
  };
  
  public TuneCloseButton(Context paramContext)
  {
    super(paramContext);
    this.adActivity = ((TuneAdActivity)paramContext);
    this.close = new ImageButton(paramContext);
    paramContext = getResources().getDisplayMetrics();
    Object localObject = Base64.decode("", 0);
    localObject = BitmapFactory.decodeByteArray((byte[])localObject, 0, localObject.length);
    localObject = new BitmapDrawable(getResources(), (Bitmap)localObject);
    this.close.setBackgroundDrawable((Drawable)localObject);
    final float f = paramContext.density;
    int i = (int)(36.0F * f + 0.5D);
    int j = (int)(8.0F * f + 0.5D);
    paramContext = new FrameLayout.LayoutParams(i, i);
    paramContext.gravity = 53;
    paramContext.setMargins(0, j, j, 0);
    this.close.setLayoutParams(paramContext);
    this.close.setOnClickListener(this.closeListener);
    addView(this.close);
    post(new Runnable()
    {
      public void run()
      {
        Object localObject = new Rect();
        TuneCloseButton.this.close.getHitRect((Rect)localObject);
        int i = (int)(12.0F * f + 0.5D);
        ((Rect)localObject).top -= i;
        ((Rect)localObject).left -= i;
        ((Rect)localObject).bottom += i;
        ((Rect)localObject).right += i;
        localObject = new TouchDelegate((Rect)localObject, TuneCloseButton.this.close);
        if (View.class.isInstance(TuneCloseButton.this.close.getParent())) {
          ((View)TuneCloseButton.this.close.getParent()).setTouchDelegate((TouchDelegate)localObject);
        }
      }
    });
  }
}


/* Location:              /Users/subdiox/Downloads/dex2jar-2.0/net.wargaming.wot.blitz-dex2jar.jar!/com/tune/crosspromo/TuneCloseButton.class
 * Java compiler version: 6 (50.0)
 * JD-Core Version:       0.7.1
 */